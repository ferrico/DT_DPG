...
void DTDPGCreateSummary::createTriggerPlots(int wh, int sec) {
  ...
  for (; hwSrcIt != hwSrcEnd; ++hwSrcIt) {  
	...
	string histoNameBXQual = triggerFolder + "/Sector" + sector.str() + "/Station" + station.str() + "/LocalTriggerPhiIn/" + (*hwSrcIt) + "_BXvsQual_W" +  wheel.str() + "_Sec" + sector.str() + "_St" + station.str();
